...
%% PARAMETERS
    ...
    signal_type = 2; % 1. p_sequence_trans, 2. p_cER, 3. speaker_chirp, 4. p_noinput, 5. air, 6. BG , 7.p_specific_f
...
%     speaker_lf = 0; %%% [Hz], signal_type = 3, output_filename, specific_f for signal_type = 7
...
%     for speaker_lf = [61 68 70 73 80 85 143 147 149 155 171 188 194 203 232] %%%

%% PARAMETERS, fixed

        if (signal_type == 1) || (signal_type == 2) || (signal_type == 3) || (signal_type == 4) || (signal_type == 5) ...
            || (signal_type == 6) || (signal_type == 7)
...
%
                elseif signal_type == 5
                    if up_or_dwn == 1
                        ...
                        dir = sprintf('G:/Analysis/pressure/%d/calc/',date);
                        fnubps = sprintf('PUpper_d%d_air%dL_hps%d_lps%d_%d.dat',lduct,flow_rate,hpsfreq,lpsfreq,num);
                        fnups = sprintf('PUpper_PS_d%d_air%dL_hps%d_lps%d_tlen%g_%d_%02u.dat',lduct,flow_rate,hpsfreq,lpsfreq,fft_mean_tlength,date,num);
                        fnups_av = sprintf('PUpper_PS_d%d_air%dL_hps%d_lps%d_tlen%g_%d_av.dat',lduct,flow_rate,hpsfreq,lpsfreq,fft_mean_tlength,date);
                        fnaxis = sprintf('PS_faxis_pressure_tlen%g.dat',fft_mean_tlength);
                        fign = sprintf('PUpper_PS_d%d_air%dL_hps%d_lps%d_tlen%g_%d_%02u',lduct,flow_rate,hpsfreq,lpsfreq,fft_mean_tlength,date,num);
                        fni = append(dir,fnubps);
                        fnps = append(dir,fnups);
                        fnps_av = append(dir,fnups_av);
                        fnaxis = append(dir,fnaxis);
                    ...
                    elseif up_or_dwn == 2
                        ...
                        dir = sprintf('G:/Analysis/pressure/%d/calc/',date);
                        fndbps = sprintf('PDown_d%d_air%dL_hps%d_lps%d_%d.dat',lduct,flow_rate,hpsfreq,lpsfreq,num);
                        fndps = sprintf('PDown_PS_d%d_air%dL_hps%d_lps%d_tlen%g_%d_%02u.dat',lduct,flow_rate,hpsfreq,lpsfreq,fft_mean_tlength,date,num);
                        fndps_av = sprintf('PDown_PS_d%d_air%dL_hps%d_lps%d_tlen%g_%d_av.dat',lduct,flow_rate,hpsfreq,lpsfreq,fft_mean_tlength,date);
                        fnaxis = sprintf('PS_faxis_pressure_tlen%g.dat',fft_mean_tlength);
                        fign = sprintf('PDown_PS_d%d_air%dL_hps%d_lps%d_tlen%g_%d_%02u',lduct,flow_rate,hpsfreq,lpsfreq,fft_mean_tlength,date,num);
                        fni = append(dir,fndbps);
                        fnps = append(dir,fndps);
                        fnps_av = append(dir,fndps_av);
                        fnaxis = append(dir,fnaxis);
...
%
                elseif signal_type == 6
                    if up_or_dwn == 1
                        ...
                        dir = sprintf('G:/Analysis/pressure/%d/calc/',date);
                        fnubps = sprintf('PUpper_d%d_BG_hps%d_lps%d_%d.dat',lduct,hpsfreq,lpsfreq,num);
                        fnups = sprintf('PUpper_PS_d%d_BG_hps%d_lps%d_tlen%g_%d_%02u.dat',lduct,hpsfreq,lpsfreq,fft_mean_tlength,date,num);
                        fnups_av = sprintf('PUpper_PS_d%d_BG_hps%d_lps%d_tlen%g_%d_av.dat',lduct,hpsfreq,lpsfreq,fft_mean_tlength,date);
                        fnaxis = sprintf('PS_faxis_pressure_tlen%g.dat',fft_mean_tlength);
                        fign = sprintf('PUpper_PS_d%d_BG_hps%d_lps%d_tlen%g_%d_%02u',lduct,hpsfreq,lpsfreq,fft_mean_tlength,date,num);
                        fni = append(dir,fnubps);
                        fnps = append(dir,fnups);
                        fnps_av = append(dir,fnups_av);
                        fnaxis = append(dir,fnaxis);
                    ...
                    elseif up_or_dwn == 2
                        ...
                        dir = sprintf('G:/Analysis/pressure/%d/calc/',date);
                        fndbps = sprintf('PDown_d%d_BG_hps%d_lps%d_%d.dat',lduct,hpsfreq,lpsfreq,num);
                        fndps = sprintf('PDown_PS_d%d_BG_hps%d_lps%d_tlen%g_%d_%02u.dat',lduct,hpsfreq,lpsfreq,fft_mean_tlength,date,num);
                        fndps_av = sprintf('PDown_PS_d%d_BG_hps%d_lps%d_tlen%g_%d_av_cER.dat',lduct,hpsfreq,lpsfreq,fft_mean_tlength,date);
                        fnaxis = sprintf('PS_faxis_pressure_tlen%g.dat',fft_mean_tlength);
                        fign = sprintf('PDown_PS_d%d_BG_hps%d_lps%d_tlen%g_%d_%02u',lduct,hpsfreq,lpsfreq,fft_mean_tlength,date,num);
                        fni = append(dir,fndbps);
                        fnps = append(dir,fndps);
                        fnps_av = append(dir,fndps_av);
                        fnaxis = append(dir,fnaxis);
...
%
                elseif signal_type == 7
                    if up_or_dwn == 1
                        ...
                        fnubps = sprintf('PUpper_speaker_hps%d_lps%d_%dHz_%dV_%gs_d%d_%d.dat',hpsfreq,lpsfreq,speaker_lf,speaker_voltage,speaker_duration,lduct,num);
                        fnups = sprintf('PUpper_speaker_PS_hps%d_lps%d_%dHz_%dV_%gs_d%d_tlen%g_%d_%02u.dat',hpsfreq,lpsfreq,speaker_lf,speaker_voltage,speaker_duration,lduct,fft_mean_tlength,date,num);
                        fnups_av = sprintf('PUpper_speaker_PS_hps%d_lps%d_%dHz_%dV_%gs_d%d_tlen%g_%d_av.dat',hpsfreq,lpsfreq,speaker_lf,speaker_voltage,speaker_duration,lduct,fft_mean_tlength,date);
                        fnaxis = sprintf('PS_faxis_pressure_tlen%g.dat',fft_mean_tlength);
                        fign = sprintf('PUpper_speaker_PS_hps%d_lps%d_%dHz_%dV_%gs_d%d_tlen%g_%d_%02u',hpsfreq,lpsfreq,speaker_lf,speaker_voltage,speaker_duration,lduct,fft_mean_tlength,date,num);
                        fni = append(dir,fnubps);
                        fnps = append(dir,fnups);
                        fnps_av = append(dir,fnups_av);
                        fnaxis = append(dir,fnaxis);
                    ...
                    elseif up_or_dwn == 2
                        ...
                        dir = sprintf('G:/Analysis/pressure/%d/calc/p%d/',date,part);
                        fndbps = sprintf('PDown_speaker_hps%d_lps%d_%dHz_%dV_%gs_d%d_%d.dat',hpsfreq,lpsfreq,speaker_lf,speaker_voltage,speaker_duration,lduct,num);
                        fndps = sprintf('PDown_speaker_PS_hps%d_lps%d_%dHz_%dV_%gs_d%d_tlen%g_%d_%02u.dat',hpsfreq,lpsfreq,speaker_lf,speaker_voltage,speaker_duration,lduct,fft_mean_tlength,date,num);
                        fndps_av = sprintf('PDown_speaker_PS_hps%d_lps%d_%dHz_%dV_%gs_d%d_tlen%g_%d_av.dat',hpsfreq,lpsfreq,speaker_lf,speaker_voltage,speaker_duration,lduct,fft_mean_tlength,date);
                        fnaxis = sprintf('PS_faxis_pressure_tlen%g.dat',fft_mean_tlength);
                        fign = sprintf('PDown_speaker_PS_hps%d_lps%d_%dHz_%dV_%gs_d%d_tlen%g_%d_%02u',hpsfreq,lpsfreq,speaker_lf,speaker_voltage,speaker_duration,lduct,fft_mean_tlength,date,num);
                        fni = append(dir,fndbps);
                        fnps = append(dir,fndps);
                        fnps_av = append(dir,fndps_av);
                        fnaxis = append(dir,fnaxis);
