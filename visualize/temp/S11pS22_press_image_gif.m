...
%     file_v=sprintf('E:/piv_output/velofield/%d/spiv_fbsc_%02u_vcl.dat',date,num);
    file_w=sprintf('E:/piv_output/S11plusS22/%d/spiv_%02u_s11ps22.dat',date,num);
    presname = sprintf('E:/pressure/%d/calc/PDown_hps20_lps300_%d_%02u.dat',date,date,num);

%     gifname=sprintf('./s11ps22_press_%d_%02u.gif',date,num);  %convert into, under the same directory
    gifname=sprintf('./s11ps22_press_%d_%02u_long.gif',date,num);  %convert into, under the same directory
...
%         end
        ...
        ax=gca;
        IMAGE = imagesc(wx,wy,w_vis(:,:),[-6000 6000]);
        ...
        c=colorbar;
        c.Ticks=[-6000 -3000 0 3000 6000];
        c.TickLabels={'-6.0','-3.0','0.0','3.0','6.0'};
        c.TickLabelInterpreter='latex';
        c.Label.FontSize = 25;
        c.Label.String = '\it \fontname{Times New Roman} S_{11}+S_{22} \rm[~10^{3} 1/s]';
