...
%     file_v=sprintf('E:/piv_output/velofield/%d/spiv_fbsc_%02u_vcl.dat',date,num);
    file_w=sprintf('E:/piv_output/strain_eig/%d/spiv_strain_eig11_%d_%02u.dat',date,date,num);
    presname = sprintf('E:/pressure/%d/calc/PDown_hps20_lps300_%d_%02u.dat',date,date,num);

%     gifname=sprintf('./strain_press_%d_%02u.gif',date,num);  %convert into, under the same directory
    gifname=sprintf('./strain_press_%d_%02u_long.gif',date,num);  %convert into, under the same directory
...
%         end
        ...
        ax=gca;
        IMAGE = imagesc(wx,wy,w_vis(:,:),[0 4000]);
        colormap(ax,jet)
        xtickformat('%.2f')
        ...
        c=colorbar;
        c.Ticks=[0 2000 4000];
        c.TickLabels={'0','2000','4000'};
        c.TickLabelInterpreter='latex';
        c.Label.FontSize = 25;
        c.Label.String = '\it \fontname{Times New Roman} \rm[1/s]';
