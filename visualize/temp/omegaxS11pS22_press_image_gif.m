...
%% Parameters 1
    ...
    frames=500;         %time steps %2000, 500
    skip_frame = 21; %the number of skip frame %0, 21
...
%     file_v=sprintf('E:/piv_output/velofield/%d/spiv_fbsc_%02u_vcl.dat',date,num);
    file_s11ps22=sprintf('E:/piv_output/S11plusS22/%d/spiv_%02u_s11ps22.dat',date,num);
...
%     gifname=sprintf('./omegaxs11ps22_press_%d_%02u.gif',date,num);  %convert into, under the same directory
    gifname=sprintf('./omegaxs11ps22_press_%d_%02u_long.gif',date,num);  %convert into, under the same directory
...
%         fseek(fid2,(first_frame-1)*Pixels*8,'bof');
        ...
        fid3 = fopen(sprintf(file_s11ps22),'r');
        ...
        fid5 = fopen(sprintf(file_omega),'r');
        fseek(fid5,(first_frame-1)*Pixels*8,'bof');
...
%         fseek(fid2,skip_frame*Pixels*8,'cof');
        ...
        s11ps22=permute(reshape(I,N),[2 1]);
        fseek(fid3,skip_frame*Pixels*8,'cof');
        
        I=(fread(fid5,Pixels,'double'));
        omega=permute(reshape(I,N),[2 1]);
        fseek(fid5,skip_frame*Pixels*8,'cof');

%         u_vis=u(4:101,:);
%         v_vis=v(4:101,:);
        s11ps22_vis=s11ps22(4:101,:);
...
%         end
        ...
        ax=gca;
        IMAGE = imagesc(wx,wy,s11ps22_vis(:,:).*omega_vis(:,:),[-10000000 10000000]);
        ...
        c=colorbar;
        c.Ticks=[-10000000 -5000000 0 5000000 10000000];
        c.TickLabels={'-1.0','-0.5','0.0','0.5','1.0'};
        c.TickLabelInterpreter='latex';
        c.Label.FontSize = 25;
        c.Label.String = '\it \fontname{Times New Roman} \omega_{z}~(S_{11}+S_{22}) \rm[~10^{7} 1/s]';
