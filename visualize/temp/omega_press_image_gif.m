...
%     file_v=sprintf('E:/piv_output/velofield/%d/spiv_fbsc_%02u_vcl.dat',date,num);
    file_w=sprintf('E:/piv_output/omega/%d/spiv_%02u_omega.dat',date,num);
    presname = sprintf('E:/pressure/%d/calc/PDown_hps20_lps300_%d_%02u.dat',date,date,num);

%     gifname=sprintf('./omega_press_%d_%02u.gif',date,num);  %convert into, under the same directory
    gifname=sprintf('./omega_press_%d_%02u_long.gif',date,num);  %convert into, under the same directory
...
%         end
        ...
        ax=gca;
        IMAGE = imagesc(wx,wy,w_vis(:,:),[-8000 8000]);
